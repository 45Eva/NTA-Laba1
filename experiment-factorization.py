...
def main():
    prime = 100000000000000000000012  # Замініть на потрібне число

    # Перевірка простого числа за допомогою тесту Міллера-Рабіна
    start_miller_rabin = datetime.now()
    is_prime_miller_rabin = Miller_Rabin(prime)
    stop_miller_rabin = datetime.now()
    print(f"Тест Міллера-Рабіна: {'просте' if is_prime_miller_rabin else 'не просте'} число.")
    print(f"Час виконання тесту Міллера-Рабіна: {stop_miller_rabin - start_miller_rabin}\n")

    # Метод пробних діленнь
    start_trial_division = datetime.now()
    divisor_trial_division = trial_division_with_pascal(prime)
    stop_trial_division = datetime.now()
    if divisor_trial_division:
        print(f"Дільник знайдено методом пробних діленнь: {divisor_trial_division}")
    else:
        print("Метод пробних діленнь не знайшов дільника.")
    print(f"Час виконання методу пробних діленнь: {stop_trial_division - start_trial_division}\n")

    # Р-метод Полларда
    start_pollards_rho = datetime.now()
    divisor_pollards_rho = pollards_rho(prime)
    stop_pollards_rho = datetime.now()
    if divisor_pollards_rho:
        print(f"Дільник знайдено методом ро Полларда: {divisor_pollards_rho}")
    else:
        print("Метод ро Полларда не знайшов дільника.")
    print(f"Час виконання методу ро Полларда: {stop_pollards_rho - start_pollards_rho}\n")

    # Метод Померанця
    start_pomerance = datetime.now()
    divisor_pomerance = pom(prime)
    stop_pomerance = datetime.now()
    if divisor_pomerance:
        print(f"Дільник знайдено методом Померанця: {divisor_pomerance}")
    else:
        print("Метод Померанця не знайшов дільника.")
    print(f"Час виконання методу Померанця: {stop_pomerance - start_pomerance}\n")
